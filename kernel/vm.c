...
int munmapWriteBack(uint64 unstart, uint64 unlen, uint64 start, uint64 offset, struct VMA *a)
{
  struct file *f = a->f;
...
int munmap(uint64 addr, int length)
{
  ...
  uint64 start = a->start, offset = a->offset, len = a->length;
  //case 1 unmap 在 start处
  if (addr == a->start)
  {
    unstart = addr;
  ...
  for (int i = 0; i < unlen / PGSIZE; ++i)
  {
    ...
    if ((pte = walk(p->pagetable, va, 0)) != 0 && (*pte & PTE_V) != 0)
    {
      ...
      {
        munmapWriteBack(va, PGSIZE, start, offset, a);
      }
      //清除映射 
