...
//
int mMapPageFault(uint64 stval, struct proc *p)
{
  ...
  {
    if (p->vma[i].used && stval >= p->vma[i].start && stval < p->vma[i].end)
    {
      a = &p->vma[i];
      break;
    }
...
//查找失败
  ...
  ilock(a->f->ip);
  if (readi(a->f->ip, 0, (uint64)mem, off, PGSIZE) <= 0)
  {
    iunlock(a->f->ip);
    return -1;
  }
